...
module.exports = {
  config: {
    name: "xnxxv2",
    aliases: ["sex", "sexy", "hot", "kanda"],
    version: "2.1.0",
    author: "BELAL ⊶ BOTX666 🪬",
    countDown: 30,
    role: 2, // শুধুমাত্র স্পেশাল পারমিশন বা এডমিনদের জন্য
    shortDescription: { en: "হট ভিডিও জেনারেটর" },
    category: "18+",
    guide: { en: "{pn}" }
  },
  ...
  onStart: async function ({ api, event, message }) {
    ...
    const waitMsgBody = `╭━━━❖✦🪬✦❖━━━╮\n  ✡️  চাঁদের 𖤍 পাহাড়  🪬\n╰━━━❖✦🪬✦❖━━━╯\n\n🔄 চাঁদের পাহাড়, দয়া করে একটু অপেক্ষা করুন... বস সিয়ামের থেকে ভিডিও কালেকশন নিয়ে আসছি! 😴🙈`;
    const loadingMessage = await message.reply(waitMsgBody);
    ...
    // ২. ভিডিও র‍্যান্ডমাইজেশন ও ডুপ্লিকেট চেকিং
    ...
    const randomVideo = availableVideos[randomIndex] || link[randomIndex];

    this.sentVideos.push(randomVideo);

    try {
      // ৩. ভিডিও ডাউনলোড ও সেন্ডিং
      const stream = await global.utils.getStreamFromURL(randomVideo);
      const latency = ((Date.now() - startTime) / 1000).toFixed(2);

      const successCaption = `╭━━━❖✦🔞✦❖━━━╮\n   ✨ 𝗫𝗡𝗫𝗫 𝗩𝗜𝗗𝗘𝗢 ✨\n╰━━━━━━⊱🔥⊰━━━━━━╯\n\n🫣 হাত মারবি না 🤭 কিন্তু পাপীর দল! 😂\n\n📊 𝗔𝗡𝗔𝗟𝗬𝗧𝗜𝗖𝗦:\n⚡ 𝗦𝗽𝗲𝗲𝗱 : ${latency}s\n🎬 𝗦𝘁𝗮𝘁𝘂𝘀 : High Quality\n\n𖤍 𝗔𝗱𝗺𝗶𝗻 : BELAL ⊶ BOTX666 🪬${sig}`;
      ...
      if (loadingMessage?.messageID) {
        setTimeout(() => api.unsendMessage(loadingMessage.messageID), 2000);
      }

    } catch (error) {
      console.error(error);
      api.setMessageReaction("❌", messageID, () => {}, true);
      if (loadingMessage?.messageID) api.unsendMessage(loadingMessage.messageID);
      message.reply("❌ দুঃখিত চাঁদের পাহাড়, ভিডিওটি পাঠাতে সমস্যা হয়েছে। গুগল ড্রাইভ লিঙ্কটি চেক করুন।");
    }
  },
};
