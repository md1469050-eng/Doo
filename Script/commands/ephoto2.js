...
module.exports = {
 ...
 // ১. লিস্ট ডিজাইন (সাইবার স্টাইল)
 if (input.toLowerCase() === "list") {
 let msg = "｢ 🌐 𝐄𝐏𝐇𝐎𝐓𝐎 𝐃𝐀𝐓𝐀-𝐁𝐀𝐒𝐄 ｣\n" + "=".repeat(25) + "\n\n";
 for (const i in availableTEMPLATES) {
 msg += `⟪${i}⟫ ❯ ${availableTEMPLATES[i]}\n`;
 }
 msg += `\n${"=".repeat(25)}\n💡 𝐔𝐬𝐚𝐠𝐞: ephoto2 [text] - [id]\n${sig}`;
 ...
 if (!text || !id) {
 return message.reply(`⚠️ সঠিক নিয়ম: ephoto2 [লিখা] - [আইডি]\nউদাহরণ: ephoto2 Moon - 7\n${sig}`);
 }

 if (isNaN(id) || id < 1 || id > 100) {
 return message.reply(`❌ ভুল আইডি! ১ থেকে ১০০ এর মধ্যে আইডি দিন।\n${sig}`);
 }

 // ২. প্রসেসিং মেসেজ ডিজাইন
 const loadingMsg = await message.reply(`💠 𝐒𝐲𝐬𝐭𝐞𝐦: জেনারেট হচ্ছে...\n📝 𝐓𝐞𝐱𝐭: ${text}\n🆔 𝐈𝐃: ${id}\n${"┄".repeat(10)}`);
 ...
 return message.reply("❌ এপিআই সার্ভারে সমস্যা হয়েছে। পরে চেষ্টা করুন।");
 }

 const resultUrl = res.data.result_url;
 const stream = (await axios.get(resultUrl, { responseType: "stream" })).data;

 await api.unsendMessage(loadingMsg.messageID);
 
 // ৩. ফাইনাল আউটপুট ডিজাইন
 return message.reply({
 body: `⚡ 𝐄𝐏𝐇𝐎𝐓𝐎 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐄𝐃 ⚡\n${"━".repeat(20)}\n✨ 𝐒𝐭𝐲𝐥𝐞: ${availableTEMPLATES[id]}\n🪬 𝐒𝐲𝐬𝐭𝐞𝐦: BOTX666\n${"━".repeat(20)}\n${sig}`,
 attachment: stream
 });
 ...
 return message.reply("❌ যান্ত্রিক ত্রুটির কারণে ব্যর্থ হয়েছে!");
 }
 }
};
